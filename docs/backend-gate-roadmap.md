...
- Remaining work: define deployment scheduler ownership separately; ranking automation/overwrite, provider ingestion, Home final-result integration, settlement extensions, and reward remain separate gates.
- Risk level: MEDIUM.
- Recommended next action: keep executable jobs limited to `noop`, `health-check`, `daily-portfolio-snapshot`, `season-ranking`, `daily-season-cycle`, and `season-settlement`; open separate gates for scheduler automation, provider ingestion, Home final-result integration, settlement extensions, and reward.
...
- Current status: operator-run season settlement MVP job implemented. It finalizes from existing settlement-date `daily_portfolio_snapshots` or existing final rankings, writes `rankType=final` `season_rankings`, and transitions `ended` seasons to `settled` through `BatchService.runJob`.
...
- Known limitations: no provider ingestion, no cron scheduler, no portfolio recalculation, no reward grant, no HTTP batch execution API, no Home authoritative final result view, and no true competition tie rank because current `season_rankings` enforces unique rank per season/date/type.
- Remaining work: Home final-result integration, reward handoff, true tie-rank schema policy if required, and any settlement extension beyond existing daily snapshots remain separate gates.
- Risk level: HIGH.
- Recommended next action: keep settlement as operator-run finalization only; do not add provider, cron, reward, or Home final-result expansion without separate gates.
...
- Not implemented: provider ingestion, cron scheduler, automatic snapshot/ranking jobs, Home authoritative final-result integration, reward, durable quote, exact order execute replay, partial fill, matching engine.
...
| Gate H: Settlement extension/final-result audit       | Decide extensions beyond existing snapshot-based settlement MVP            | Season settlement MVP implemented; Gate F/G recommended for automation          | docs only                                                   | provider/reward/schema code unless explicitly approved                                             | no build required; maybe `prisma validate`                                                 | final price/FX evidence, Home final result, recovery, reward handoff, or tie-rank policy undecided                      | extension scope and test matrix accepted                                                 | `Gate H - Settlement extension readiness audit`               | MEDIUM         | Gate F, Gate G     |
...
- Settlement extensions until Gate H/I fixes final price evidence, Home final-result integration, true tie rank, or recovery behavior.
...
| 3. Season settlement MVP                              | HIGH       | VERY HIGH                  | MEDIUM                     | LOW                 | MEDIUM                               | Enough for existing snapshots | Completed for MVP               | DONE FOR MVP         | Operator-run settlement can now finalize from existing daily snapshots without provider keys, cron, or reward.                              | Home final result, true tie rank, reward handoff remain separate                      | Keep settlement MVP bounded; open separate extension/reward gates         |
