...
- Batch job execution foundation is implemented with `BatchJobRun`/`BatchJobStatus`, `BatchService`, an operator-only noop/health-check script, operator-run `daily-portfolio-snapshot` and `season-ranking` jobs, an operator-run `daily-season-cycle` orchestration job, an operator-run `season-settlement` MVP job, an operator-run `final-tier-assignment` MVP job, and an operator-run `reward-grant` marker MVP job. It is not a cron scheduler, provider ingestion, or actual reward fulfillment implementation.
- Home settled final-result read model is implemented from existing `rankType=final` `season_rankings`; final tier assignment and reward grant marker now have operator-run MVP jobs, while actual payment/badge/trophy fulfillment remains a separate gate.
...
- Known limitations: no cron scheduler, no provider ingestion job, no actual reward/payment fulfillment job, no ranking overwrite/regeneration policy, and no batch execution HTTP API because admin roles are not implemented. The daily snapshot job uses existing DB `admin_manual` evidence only and does not create rankings. The season ranking job reads existing `daily_portfolio_snapshots` only and does not create snapshots. The daily season cycle job only orchestrates those two child services in order. The season settlement MVP job reads existing `daily_portfolio_snapshots`, creates `rankType=final` rankings, and transitions `ended` seasons to `settled`; it does not recalculate portfolios, call providers, run cron, or grant rewards. The final tier assignment MVP job reads existing final rankings and updates only participant `finalRank`/`finalTier`; it does not grant rewards or change ranking policy. The reward grant marker MVP job updates only `SeasonParticipant.rewardGrantedAt`; it does not calculate reward amounts or create payment/badge/trophy rows.
- Remaining work: define deployment scheduler ownership separately; ranking automation/overwrite, provider ingestion, settlement extensions beyond final tier assignment, true competition tie rank, reward amount policy, and actual reward/payment/badge/trophy fulfillment remain separate gates.
- Risk level: MEDIUM.
...
- Known limitations: provider-backed price freshness evidence, automatic snapshot/ranking generation, actual reward/payment/badge/trophy fulfillment, and settlement extensions remain unavailable/blocked. Equity chart reads existing snapshots only and does not generate them. Missing final rankings remain unavailable with no live valuation fallback. Missing `finalTier` remains `FINAL_TIER_UNAVAILABLE` until the operator-run final tier assignment job is executed. Missing `rewardGrantedAt` remains `REWARD_NOT_GRANTED` until the operator-run reward grant marker job is executed.
- Remaining work: provider ingestion gate, scheduler daily snapshots, ranking automation, actual reward fulfillment handoff, and settlement extensions beyond existing final rankings/final tier assignment/reward grant marker.
- Risk level: MEDIUM.
...
- Known limitations: no provider ingestion, no cron scheduler, no portfolio recalculation, no actual reward/payment/badge/trophy fulfillment, no HTTP batch execution API, and no true competition tie rank because current `season_rankings` enforces unique rank per season/date/type. Final tier assignment and reward grant marker exist only as separate operator-run MVP jobs.
...
- Current status: operator-run reward grant marker MVP job implemented. It requires a `settled` season and final-assigned participants, then sets `SeasonParticipant.rewardGrantedAt` only where it is still null.
- Implemented files: `src/batch/reward-grant-job.service.ts`, `src/batch/reward-grant-job.types.ts`, `src/batch/batch-admin-runner.ts`, `scripts/admin-run-batch-job.ts`.
- Source of truth: `docs/current-status.md`, `docs/batch-job-foundation.md`, `docs/home-api-contract.md`, `prisma/schema.prisma`.
- Existing tests: `src/batch/reward-grant-job.service.spec.ts`, `src/batch/batch-admin-runner.spec.ts`, `src/home/home.service.spec.ts`.
- Known limitations: no reward amount calculation, reward/payment/badge/trophy schema or rows, point wallet, external transfer, provider ingestion, cron scheduler, HTTP batch execution API, or custom fulfillment policy. `rewardPolicyJson` is not used for actual amount/item fulfillment in this MVP.
- Remaining work: define reward amount/schema/fulfillment policy if product requires payment/points/badges/trophies.
- Risk level: HIGH.
...
| Gate J: Reward/badge/trophy fulfillment implementation | Fulfill rewards from settled final result beyond the marker MVP            | Reward grant marker MVP accepted; Gate I if final-result extension is required                                                                               | reward/payment/badge/trophy service/schema/tests/docs if approved | settlement recalculation unless explicit                                                           | unit/integration/idempotency                                                               | no reward policy/schema/user-facing contract                                                                            | fulfillment is idempotent and tied to settled evidence                                   | `Gate J - Reward badge trophy fulfillment implementation`     | HIGH           | Settlement MVP     |
...
| 3. Season settlement MVP                              | HIGH       | VERY HIGH                  | MEDIUM                     | LOW                 | MEDIUM                               | Enough for existing snapshots | Completed for MVP               | DONE FOR MVP         | Operator-run settlement can now finalize from existing daily snapshots without provider keys or cron; final tier assignment and reward grant marker MVP can consume final rankings/final assignments. | True tie rank, advanced tier policy, and actual reward fulfillment remain separate    | Keep settlement/final-tier/reward-marker MVP bounded; open separate extension/fulfillment gates |
