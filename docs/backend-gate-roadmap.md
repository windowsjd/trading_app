...
- Known limitations: access-token blacklist/revocation, cookie auth, server-side session auth beyond refresh sessions, issuer/audience policy, refresh-token reuse theft-response hardening, and admin/operator account management APIs are not implemented.
- Remaining work: keep Bearer access-token guard behavior stable; defer access-token blacklist/cookie-session/auth-hardening and admin/operator account management to separate gates.
- Risk level: MEDIUM.
...
- Known limitations: no admin account/role management API, no provider ingestion trigger API, no batch run HTTP API, no reward fulfillment trigger API, and no scheduler/cron. `GET /api/v1/operator/me` is read-only and does not write audit rows.
- Remaining work: Operator/Admin Account Management Gate or Scheduler/Ops Foundation Gate. This remains separate from provider_api source eligibility.
- Risk level: MEDIUM.
...
- Known limitations: no provider cron scheduler, no admin HTTP provider ingestion API, no Binance WebSocket ingestion, no KIS REST current-price ingestion, no KIS orderbook/hoga ingestion, no provider_api source eligibility switch for quote/execute/valuation/snapshot/ranking/settlement, no new provider metadata schema, and no provider-backed final settlement policy.
- Remaining work: provider_api eligibility gate, source priority, stale thresholds by workflow, KIS live smoke evidence, KIS REST quote endpoint mapping only if a future gate needs it, scheduler/deployment ownership, and provider outage monitoring.
- Risk level: MEDIUM.
...
| Gate D Asset price provider ingestion     | FOUNDATION IMPLEMENTED for Binance public crypto and KIS WebSocket stock row insertion; source eligibility gated | Binance public REST ticker can insert USD-equivalent crypto snapshots for existing mapped assets; KIS WebSocket can insert KRX `H0STCNT0` and US `HDFSCNT0` trade-price snapshots for existing mapped assets | source eligibility, KIS live smoke evidence, timestamp freshness acceptance, plan/terms approval, separate domestic/US/crypto freshness tests |
...
- KRX and US stock provider_api quote/execute until source eligibility, freshness, live smoke evidence, and product/terms decisions exist.
