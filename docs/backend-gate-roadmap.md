...
- Provider ingestion foundation is implemented for explicit operator-run ExchangeRate-API USD/KRW and Binance public crypto market data snapshot insertion, plus KIS market data skeleton/config/redaction/watchlist parsing. It is not a cron scheduler, admin HTTP API, WebSocket ingestion, KIS quote ingestion, or any real trading/account/balance integration.
...
- Current status: foundation implemented for explicit operator-run provider_api snapshot insertion. ExchangeRate-API can create USD/KRW `fx_rate_snapshots`; Binance public REST ticker can create USD-equivalent crypto `asset_price_snapshots` for existing unambiguous `BINANCE` crypto assets. KIS is skeleton-only for market data config, redaction, watchlist, token parsing, approval key parsing, and explicit-path low-level client behavior.
- Implemented files: `src/providers/**`, `scripts/provider-ingest-fx-rate.ts`, `scripts/provider-ingest-binance-prices.ts`, `src/app.module.ts`.
...
- Existing tests: provider config, secret redaction, raw payload truncation, ExchangeRate parsing/ingestion, Binance parsing/ingestion, KIS watchlist, KIS token/approval parsing, and KIS skeleton no-real-trading-surface guard under `src/providers/**/*.spec.ts`.
- Known limitations: no provider cron scheduler, no admin HTTP provider ingestion API, no WebSocket ingestion, no KIS live quote ingestion, no provider_api source eligibility switch for quote/execute/valuation/snapshot/ranking/settlement, no new provider metadata schema, and no provider-backed final settlement policy.
- Remaining work: provider_api eligibility gate, source priority, stale thresholds by workflow, WebSocket ingestion design, KIS quote endpoint mapping after official path confirmation, scheduler/deployment ownership, and provider outage monitoring.
- Risk level: MEDIUM.
- Recommended next action: keep provider_api rows isolated as stored evidence until the eligibility gate explicitly changes read paths.
...
| Gate C FX provider ingestion              | FOUNDATION IMPLEMENTED for ExchangeRate-API USD/KRW row insertion; source eligibility gated | ExchangeRate-API can insert provider_api USD/KRW snapshots; OANDA/Twelve Data remain historical candidates for later review                                     | provider_api source eligibility, source priority, stale thresholds, contract/cost/polling approval                                                                                                                       |
| Gate D Asset price provider ingestion     | FOUNDATION IMPLEMENTED for Binance public crypto row insertion; BLOCKED for KRX quote/execute | Binance public REST ticker can insert USD-equivalent crypto snapshots for existing mapped assets; Twelve Data US stock and KRX remain later gates               | source eligibility, KIS/Twelve Data official quote mapping, symbol mapping, timestamp proof, plan/terms approval, separate domestic/US/crypto freshness tests                                                             |
...
- Provider_api source eligibility until source priority, freshness, and terms decisions are accepted.
...
| Binance crypto fixture                | GO for fixture capture and row insertion foundation; STOP for source eligibility | Crypto provider target changed to Binance USD-settled crypto; public REST ticker foundation now stores USD-equivalent provider_api rows for existing mapped assets                             | Verify freshness/source eligibility, terms, and financial read/write path behavior before provider_api consumption |
...
- Provider_api source eligibility remains `BLOCKED` until source priority, freshness, and terms decisions are accepted.
- Crypto provider_api source eligibility must not start until Binance timestamp/freshness/sourceType eligibility tests are accepted.
...
| Gate D mapping blockers           | STOP for source eligibility                               | Binance fixture response shape exists and row insertion foundation exists, but financial read/write path policy decisions are missing           | effectiveAt mapping, sourceType eligibility, price-field decision, terms approval                                      |
...
| ----------------------------------------------------- | ---------- | -------------------------- | -------------------------- | ------------------- | ------------------------------------ | ----------------------------- | ------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 1. Provider ingestion foundation                      | HIGH       | HIGH                       | MEDIUM                     | HIGH                | MEDIUM                               | Enough for ExchangeRate/Binance foundation | Completed for foundation       | DONE FOR FOUNDATION  | Provider rows can be inserted for ExchangeRate-API USD/KRW and Binance public crypto without changing financial read/write eligibility.                                                                            | source eligibility, cost/contract owner, polling/timestamp/freshness checklist        | Provider API Source Eligibility Gate                                                           |
