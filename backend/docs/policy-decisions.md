...
- 지정가 자동 체결은 아래 Scheduler Matching 계약을 따른다. Path A는 fresh provider snapshot 가격, Path B는 적격 마감 5분봉 터치를 근거로 order.limitPrice를 사용한다.
  근거: 등록은 예약만 만들며, 자동 체결의 가격·evidence 정책은 경로별로 다르다.
...
- 예약금: `gross=round8(limitPrice×qty)`, `fee=round8(gross×feeRate)`, `reserved=round8(gross+fee)` — 시장가 매수 netAmount와 동일한 반올림 체인. 등록 시점 feeRate는 `orders.reservation_fee_rate`에 영구 저장된다(현재 Path A/B 체결도 이 rate를 사용).
- 수수료율 고정(Quote 시점): 아래 예약금 검증은 매수 기준이다. 매수 예약 근거를 durable Quote의 `quotedFeeRate/quotedGrossAmount/quotedFeeAmount/quotedReservedAmount`에 저장한다. 매도는 `quotedReservedAmount` 대신 `quotedNetAmount`를 검증하고 수량을 예약하며 같은 quote-time fee를 Order에 보존한다. Create는 이 저장값을 그대로 예약하고 현재 `Season.tradeFeeRate`를 다시 읽지 않는다. Quote 이후 운영자가 시즌 수수료율을 바꿔도 Quote 응답 예약금 = 실제 wallet reservedAmount 증가액 = `Order.reservedAmount`가 모두 동일하며, `Order.reservationFeeRate`도 Quote 시점 rate다. Create는 저장값을 재검증한다(전부 non-null, 음수 아님, rate가 [0,1], gross/fee/reserved가 Quote의 limitPrice×quantity에서 canonical 반올림 체인으로 재도출 가능). 실패 시 `QUOTE_RESERVATION_BASIS_INVALID`(409)로 거절하며 현재 시즌 수수료율로 대체 계산하는 fallback은 두지 않는다 — 조용한 재가격 산정이야말로 이 고정이 막으려는 실패다.
- 미체결 주문 금액 필드 의미: `grossAmount`/`feeAmount`/`netAmount`/`executedPrice`/`executedAt`는 **실제 체결 결과**만 의미한다. 미체결 `submitted`·`canceled` 지정가 주문은 이 다섯 필드가 모두 null이다. 미체결 주문의 금액은 `reservedAmount`(체결금액이 아니라 미체결 예약금)와 `reservationFeeRate`, 그리고 등록 전 단계에서는 Quote의 `quoted*` 예상값으로 제공한다. 시장가 executed 주문의 세 금액 의미는 그대로 유지한다. 근거: 예약 추정치를 체결 결과 컬럼에 쓰면 미체결 주문이 체결된 것처럼 읽히고, 예약액과 실제 체결액을 구분할 수 없게 된다.
- Create 동시성: transaction 밖 season/participant 검사는 빠른 오류 반환용 보조 검증일 뿐이다(그 사이에 운영자가 참가자를 제외하거나 시즌을 종료할 수 있다). 금융 정확성은 transaction 안에서 잠근 행의 재검증이 보장한다. 시즌 지정가 Create의 lock 순서는 `Quote(FOR UPDATE) → Season(FOR SHARE) → TradingAccount(FOR SHARE) → SeasonParticipant(FOR SHARE) → CashWallet/Position 예약 → Order(insert)`다. `season-trading-lock.ts`가 잠금 대상 조회 후 잠긴 행들의 user/season/account 연결을 재검증한다.
  - Quote와 인가 lock 뒤, 예약 쓰기 전에 `clock_timestamp()`를 조회한다. transaction 시작 시각에 고정되는 `now()`/`CURRENT_TIMESTAMP`는 사용하지 않는다. Quote TTL, Season.start/end, 주식시장 세션을 이 시각으로 재검증하고 Order submitted/created/updated 시각도 동일 값을 쓴다.
  ...
  - ranking/settlement는 Season을 먼저 잠그고, 운영자 제외는 Account → Participant → 주문 cleanup 순서다. 시즌 시장가/FX/지정가 체결도 Season → Account → Participant 인가 순서를 사용하되 participant valuation을 쓰므로 Participant는 처음부터 `FOR NO KEY UPDATE`다. 일반모드는 Account `FOR UPDATE`로 TWR/금융 변경을 직렬화한다. Quote가 있는 create/execute는 Quote를 먼저 잠그며 지정가 자동 체결은 인가 → Order → 예약 settle 순서다. 취소·cleanup은 Order → CashWallet/Position으로 예약을 해제한다.
  - 부수 효과: 시즌이 ended가 된 뒤에는 신규 예약이 생길 수 없으므로, settlement는 사전점검에 더해 Season을 잠근 정산 transaction 안에서도 open order·cash/position reservation을 재검사한다.
...
제거되었고, 자동 체결은 **스케줄러 폴링**으로 재구현했다. PostgreSQL submitted
orders를 조회하고, 작업 소유권은 기존 PostgreSQL OpsJobLockService +
ops_job_locks lease로 관리한다. 갱신/ownership loss 경계는 Ops 계약을 따른다. 실제 거래소 주문도 아니다.
...
- 체결 tx(주문별 독립): mode별 인가 lock(위 순서) → Order(FOR UPDATE) →
  CashWallet/Position 예약 settle·금융/원장 반영. 취소·cleanup은 인가 lock 없이 Order부터 잠근다. status=submitted·mode별 account gate·
...
- 취소·체결 경합: 둘 다 Order row lock + status=submitted 조건 → 정확히 한쪽 승리. 체결
  ...
  해제 없음. 시즌 종료/정산 경합은 먼저 Season 인가 lock으로 중재한다.
...
- 배치: `LIMIT_ORDER_MATCH_BATCH_SIZE`(기본 200)는 체결 시도 예산이다. 후보는 `(submittedAt, id)` keyset cursor로 cycle 간 전진하고 EOF에서 순환하며, 별도 scan 예산을 사용한다. 앞쪽 미체결/실패 후보가 뒤쪽 후보를 영구 차단하지 않는다. 주문별 오류는 격리한다.
...
- general 시장가 durable quote는 quote 시점의 `GENERAL_TRADE_FEE_RATE`를 기존 `Quote.quotedFeeRate`에 고정하고 create/execute가 그 값을 사용한다. provider 가격은 기존처럼 execute-time repricing한다. 구버전의 null fee general market quote는 현재 config를 조용히 적용하지 않고 409 `QUOTE_MISMATCH`로 재견적한다. 시즌 시장가도 quote 시점 `Season.tradeFeeRate`를 pin한다. 양 모드의 새 시장가·FX quote는 모두 `quotedFeeRate`를 저장하며, execute는 가격/환율만 재산정한다. legacy season null fee quote만 당시 호환 fallback을 유지하고 general null/잘못된 pin은 `QUOTE_MISMATCH`다. 지정가는 Quote 예약 근거 → `Order.reservationFeeRate`를 계속 사용한다.
...
- 서버는 현재 선택 계정/모드를 어디에도 저장하지 않는다(JWT claim·세션·User 컬럼·전역 singleton 금지). 현재 account-scoped 거래 API는 경로로 accountId를 명시하고 요청마다 `TradingAccountAccessService`로 소유권을 재검증한다.
...
- account-scoped 취소는 Order row를 orderId + TradingAccount.userId로 잠근 뒤 canonical account scope를 검사한다. 같은 사용자의 다른 정상 계정 주문은 404 `ORDER_NOT_FOUND`; 요청 계정과 order relation의 모순은 500 `TRADING_ACCOUNT_SCOPE_MISMATCH`다. 시즌 주문의 participant link는 시즌 domain integrity로 별도 검증하며 금융 소유권 fallback으로 사용하지 않는다. 오류 시 주문·예약금은 불변이다.
...
- legacy 시장가 replay 조회는 실제 DB unique와 범위가 같은 축만 사용한다(고유 `Order.quoteId` + 사용자 소유권). `userId + idempotencyKey` 같은 넓은 조회는 금지한다.
  근거: `(tradingAccountId, idempotencyKey)`가 DB unique다. 더 넓은 user/key 조회는 다른 계정·시즌 주문을 replay하거나 정상 요청을 충돌로 만든다.
- 원자 지갑 UPDATE가 0행이면 wallet id 단독으로 재조회해 원인을 분류한다: 행 없음 → 기존 not-found/잔액 오류, account 누락·account 불일치·통화 불일치 → 각각의 구조화된 500, 금액 조건 미달 → 기존 잔액/예약금 오류, 전부 정상 → 실제 동시성 CONFLICT. scope는 항상 금액보다 먼저 검사하며, 진단은 읽기 전용이다. 시장가 debit/credit, 지정가 reserve/settle/release(취소·만료·운영자 제외 cleanup 공용), FX source debit/target credit 전 경로에 동일하게 적용한다.
...
> Historical migration notes: 아래 nullable/additive/dual-write와 NOT NULL 후속 계획은 당시 단계다. 현재 SeasonRanking.tradingAccountId도 required이며 participant relation은 시즌 도메인 식별자로 유지한다. 정산의 모든 season account closure와 final scope 검증은 구현되어 있다.
...
- ranking refresh·daily ranking job·settlement는 모두 같은 `seasons` row를 `FOR UPDATE`로 잠가 직렬화한다. 현재 `RankingRefreshService`는 프로세스 안에서 요청을 병합하고 trailing refresh를 예약한다. writer는 Season lock 아래 generation·참가자 집합을 재검증해 오래된 계산의 덮어쓰기를 막고, ranking reader는 RepeatableRead transaction으로 한 응답의 세대를 일치시킨다. Redis 분산락·큐는 도입하지 않는다.
