...
- 기능 플래그: `LIMIT_ORDER_ENABLED`(기본 false). 허용값은 `true`/`false`/`1`/`0` 뿐이며 trim 후 대소문자 무시(`TRUE`, `False` 허용). 변수를 아예 두지 않으면 기본값 false. 그 외 값(`yes`, `enabled`, `tru`, 명시적 빈 문자열 등)은 조용히 false로 처리하지 않고 **startup validation error로 부팅을 실패**시킨다(`ConfigModule.forRoot({ validate })` → `src/common/env-validation.ts`). 근거: 운영자가 켰다고 믿는 플래그가 오타 때문에 조용히 꺼져 있는 상태가 가장 위험하다. 파서는 `src/orders/limit-order.config.ts` 하나만 두고 startup 검증과 런타임 판정이 동일 함수를 쓴다 — 서비스 메서드마다 별도 parser를 만들지 않는다. off면 지정가 quote/create만 차단(`LIMIT_ORDER_DISABLED`), 취소/시즌 cleanup/참가자 제외 cleanup/정산 예약 guard는 항상 동작한다. 2차 자동 체결 구현 전 production 활성화는 권장하지 않는다.
