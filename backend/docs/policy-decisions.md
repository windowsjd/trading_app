...
- Create idempotency 순서: 이미 commit된 limit Create의 replay는 `LIMIT_ORDER_ENABLED`와 `LimitOrderCreateService` wiring 검사보다도 **먼저** 수행한다. flag는 신규 등록을 막는 장치이지 이미 만들어낸 응답을 회수하는 장치가 아니며, replay가 흡수해야 할 retry 폭주는 바로 emergency rollback이나 wiring 실패 직후에 발생한다. 신규 quote는 flag off면 여전히 `LIMIT_ORDER_DISABLED`, 미구성 인스턴스면 `LIMIT_ORDER_SERVICE_UNAVAILABLE`을 받는다. replay는 DB write를 전혀 하지 않으므로 추가 Order·예약이 생기지 않고, active season을 읽지 않으므로 시즌 종료 후에도 동작한다.
- Create idempotency 범위: replay 조회 키를 `(quoteId, idempotencyKey, 소유자)` + `orderType=limit`으로 고정한다. DB가 key에 강제하는 유일성은 `(seasonParticipantId, idempotencyKey)` — 즉 **시즌 참가 단위**이므로 `(userId, idempotencyKey)` 조회는 제약보다 넓었고 `createdAt desc`로 스스로 tie-break해야 했다. 그 결과 다른 시즌에서 같은 key를 재사용하면(스키마상 허용) 시즌 1 재시도가 시즌 2 Order로 해석돼 잘못된 `ORDER_IDEMPOTENCY_CONFLICT`가 났다. `Order.quoteId`는 unique이고 지정가 Create는 항상 사용자 소유·1회 소비 quote를 갖기 때문에, 이 범위는 실제 DB 제약과 **정확히 같다**. 시즌 간 같은 key 재사용은 정상이며 각 요청은 자기 quote의 Order로 replay된다. hash 불일치, 같은 quote의 다른 key, 시장가가 소비한 quote, 타 사용자 quote는 모두 conflict다.
- retention gap 폭발 반경: 세 신호를 blast radius로 분리한다. 공유 market-time watermark가 retention 지평보다 뒤진 경우만 **global**이다(모든 자산이 공유하는 단일 위치라 어느 자산의 손실인지 귀속 불가). deferred entry의 candle row 소멸과 미스캔 matchable candle의 retention 초과는 각각 정확히 한 자산을 지목하므로 그 자산의 completion checkpoint에 **asset gap**으로 기록하고 해당 자산의 신규 Quote/Create만 차단한다. 이전에는 둘 다 공유 checkpoint에 기록돼 무관한 전 자산의 신규 지정가 주문을 멈췄다. 또한 sweep당 자산별로 1행씩 기록한다 — global `LIMIT 1` 한 행은 가장 오래된 자산의 손실이 다른 자산의 손실을 가려, 그 자산들이 이미 눈먼 안전망 위에서 주문을 계속 받게 했다. `LIMIT_ORDER_CANDLE_ASSET_GAP_BATCH_SIZE`로 write burst만 제한하고 초과분은 다음 sweep에서 기록하며 truncation을 로깅한다. asset gap은 `gapReason`/`gapMarketCandleId`/`gapCandleIngestSeq`를 별도 컬럼에 sticky하게 남긴다(`degradedReason`은 completion supervisor가 매 pass 덮어쓰므로 알람 근거로 쓸 수 없다).
- deferred revision provenance: `candle_ingest_seq` 값만으로는 그 revision을 **관측**한 것인지 backfill이 **추정**한 것인지 구분할 수 없다. 20260723230000의 backfill은 candle의 현재 revision을 기록하는데, revision 1에서 실패해 `permanent`로 park된 row에 migration 직전 correction된 revision 2가 찍히면 forward scan(`tracked >= current`)과 retry loop(`status='deferred'`만 조회) 양쪽에서 동시에 배제돼 revision 2가 **영구 누락**된다. legacy row와 신규 row는 데이터상 동일하므로 provenance는 재도출 불가이다. `legacy_unknown`은 revision을 NULL로 되돌려 아무것도 억제하지 않게 하고 즉시 재검증 대상으로 되살리며, candle row가 이미 사라진 `legacy_orphan`은 재검증 자체가 불가능하므로 되살리지 않고 해당 자산만 차단한 채 운영자 판단을 기다린다. `first_deferred_at`은 **보존**한다 — asset health gate가 backlog age를 재는 기준이라 초기화하면 재검증이 끝나지 않았는데 자산이 열려버린다. (legacy 오판은 status guard가 보호하는 sweep 1회 비용, current 오판은 체결 누락 — 두 오류는 비대칭이다.)
- provenance 판정은 clock-skew 독립: 20260724120000이 쓴 "row `created_at` < migration `finished_at`" 경계는 **서로 다른 clock domain의 비교**였다. `created_at`은 애플리케이션이 넘긴 wall clock이고 migration ledger는 DB clock이라, 애플리케이션 시계가 DB보다 앞서면 진짜 legacy row가 modern으로 오인돼 추정 revision이 계속 신뢰되고 corrected revision이 영구 누락된다 — 원래 결함이 시계 설정에 조건부로 재현되는 셈. 20260724200000은 시간 비교를 버리고 "runtime이 그 revision을 candle에서 **관측했다는 durable 증거**(`revision_verified_at`)가 있는가"로 판정한다: `revision_verified_at IS NULL AND revision_migrated_at IS NULL`인 row는 `created_at`이 어느 방향으로 틀렸든 보수적으로 재검증 대상이다. application `created_at`/`updated_at`/provider timestamp/process wall clock은 판정에 쓰지 않는다. `revision_verified_at` 자체는 DB clock(`CURRENT_TIMESTAMP`)으로만 기록한다 — provenance authority 시각에 애플리케이션 시계를 넣으면 같은 모호성이 재생산된다. 관측이 검증된 row(`revision_verified_at IS NOT NULL`)는 migration 재적용에도 status/attempt/retry 일정이 불변이다.
- asset-gap batch 전진: `recordAssetGap`은 sticky(first-detection-wins)이므로, 이미 gap이 기록된 자산을 orphan/unscanned query에서 **제외하지 않으면** 같은 가장 오래된 `batchSize`개 자산이 매 sweep의 batch를 no-op으로 소진해 상한 초과 자산이 영원히 기록되지 않는다. 두 probe 모두 `gap_detected_at IS NOT NULL`인 자산을 DB에서 제외하고, 이번 sweep이 이미 건드린 자산 집합도 제외한다(query snapshot이 아직 못 보므로). batchSize는 write-burst 제한일 뿐 정확성 제한이 아니며 n개 문제 자산은 `ceil(n/batchSize)` sweep 안에 전부 기록된다. `recordAssetGap`은 `'inserted' | 'already_exists'`를 반환하고 summary `assetGapsDetected`는 실제 신규 기록만 센다 — sticky 정책과 기존 evidence는 불변.
- asset-gap budget은 **sweep 전체 상한**: `LIMIT_ORDER_CANDLE_ASSET_GAP_BATCH_SIZE`는 probe별 LIMIT이 아니라 한 sweep이 기록하는 신규 asset gap의 **총** 상한이다(probe별이면 한 sweep에 최대 2×batchSize가 기록돼 설정·문서의 write-burst 계약과 어긋난다). orphan probe가 budget을 먼저 소비하고 남은 만큼만 unscanned probe에 전달하며(0이면 query 자체를 생략), 실제 신규 insert만 budget을 감소시킨다 — raced `already_exists`는 slot을 반환하고 probe는 남은 budget 한도 안에서 아직 미기록 자산을 bounded re-query loop로 다시 찾는다(새 candidate가 없으면 즉시 종료, local seen set으로 무한 반복 방지). 한 sweep의 신규 기록 수는 항상 0..batchSize이고, budget 소진·잔여 추정은 `limit_order_candle_asset_gap_budget` 한 줄로 회계한다.
- deferred revision provenance 불변식: revision **값**과 그 값을 **관측했다는 증거**는 다른 것이다. `upsertDeferred`는 `revisionObserved` boolean(컴파일 타임 union + 런타임 assertion)으로 이를 분리한다 — observed=true(이번 호출이 candle row를 읽음)만 `current` + `revision_verified_at(CURRENT_TIMESTAMP)`을 만들 수 있고, observed=false는 verified를 절대 새로 쓰지 않으며 stored revision을 이동시키지 않고 forced legacy 상태에서 forensic seq만 보존한다(과거 writer는 non-NULL seq만으로 observed를 추론해 missing-candle retry가 park하는 `legacy_orphan`에 verified를 찍었다 — 그 관측은 일어난 적이 없다). 허용 형태는 `current(seq NOT NULL, verified NOT NULL)` / `legacy_unknown(seq NULL, verified NULL)` / `legacy_orphan(verified NULL, seq는 forensic으로 자유)`뿐이며, 20260724230000이 기존 invalid row를 보수적으로 보정(unverified current는 candle 유무에 따라 legacy_unknown 재개 또는 legacy_orphan park, legacy의 unearned verified는 stamp만 제거)한 뒤 DB CHECK로 동결한다. 방어는 3중: 타입/런타임 assertion, DB CHECK, 그리고 constraint 이전 데이터를 위한 소비자 방어 — scan suppression은 `current AND verified NOT NULL`인 entry만 revision coverage로 인정하고(legacy_orphan forensic seq나 미검증 concrete 값은 candle을 억제하지 못함), health gate의 legacyReviewCount는 `state <> 'current' OR verified IS NULL OR seq IS NULL`로 invalid current를 legacy review로 계산해 해당 asset만 fail-closed한다.
- replay 조회 소유권: quote-scoped replay 조회는 `quoteId`만으로 row를 읽고 앱에서 소유자를 비교하지 않는다 — 그 형태는 타 사용자의 소비된 quoteId에 즉시 `ORDER_IDEMPOTENCY_CONFLICT`를 주고 존재하지 않는 quoteId는 다른 오류를 줘, **남의 quote 소비 여부를 응답 차이로 탐지**할 수 있게 했다. 소유권(`seasonParticipant.userId`)을 query 자체에 넣어 타 사용자 row는 아예 읽히지 않으며, 타 사용자 quote와 존재하지 않는 quote는 동일한 gate 경로를 지나 동일한 user-scoped `QUOTE_NOT_FOUND`로 끝난다. 자기 quote의 key 불일치·시장가 소비는 종전대로 정확한 conflict다.
