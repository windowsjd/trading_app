...
- asset-gap batch 전진: `recordAssetGap`은 sticky(first-detection-wins)이므로, 이미 gap이 기록된 자산을 orphan/unscanned query에서 **제외하지 않으면** 같은 가장 오래된 `batchSize`개 자산이 매 sweep의 batch를 no-op으로 소진해 상한 초과 자산이 영원히 기록되지 않는다. 두 probe 모두 `gap_detected_at IS NOT NULL`인 자산을 DB에서 제외하고, unscanned probe는 같은 sweep의 orphan pass가 방금 기록한 자산 집합도 제외한다(query snapshot이 아직 못 보므로). batchSize는 write-burst 제한일 뿐 정확성 제한이 아니며 n개 문제 자산은 `ceil(n/batchSize)` sweep 안에 전부 기록된다. `recordAssetGap`은 `'inserted' | 'already_exists'`를 반환하고 summary `assetGapsDetected`는 실제 신규 기록만 센다 — sticky 정책과 기존 evidence는 불변.
