...
- `GET /api/v1/fx/rates/current` selects provider rows by the same source priority using display freshness: 7200 seconds by default, overridden by a positive integer `PROVIDER_FX_RATE_DISPLAY_FRESHNESS_SECONDS`. `refresh=true` may check Korea EXIM at the existing 300-second refresh threshold; response selection uses display freshness and the clock after refresh completes. Stale provider rows are not returned ahead of fresh fallback providers; if all provider rows are stale, it uses approved `admin_manual` fallback or returns `FX_RATE_UNAVAILABLE`. Quote freshness remains 300 seconds by default and execute remains 60 seconds.
- Each successful provider fetch is a new observation: Korea EXIM and ExchangeRate-API insert a new row with the fetch's `receivedAt` as `capturedAt`, even when rate/effectiveAt are unchanged. Existing observations are never updated. Korea EXIM on-demand refresh still reuses sufficiently fresh stored observations without fetching or writing.
