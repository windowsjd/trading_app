...
| ---------------- | ------------------------------------------------------------------ | ------------------------------------------ | ------------------------------ | ----------------------------------------- | --------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `admin_manual`   | Bootstrap, manual correction, emergency/provider outage fallback   | Yes, current/fallback                      | No default execute fallback    | Yes, current/fallback                     | Yes, current manual CLI input                 | Indirectly through generated snapshots/rankings | Yes when latest valid stored evidence is selected at `Season.endAt` | Explicit operator action, meaningful `effectiveAt`, no fake/static/sample data, freshness rules still apply where defined | Implemented for FX, asset price, quote/read valuation consumers, settlement valuation, and manual CLIs; forbidden for execute fallback                        |
| `provider_api`   | Real-time or near-real-time provider source for approved workflows | Yes for approved read-only/quote workflows | Yes for durable quote execute workflows | Yes for approved live valuation workflows | Yes for operator-run daily snapshot valuation | Indirectly through snapshots/rankings only      | Yes when latest valid stored evidence is selected at `Season.endAt` | Provider timestamp required, sourceName required, stale/missing/outage rejected, tests required                           | Row insertion foundation exists for ExchangeRate-API, Binance, and KIS; allowed only for explicit workflow allowlist including quote/read, execute, daily snapshot, and settlement |
...
| -------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | --------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------- |
| FX USD/KRW           | Korea EXIM provider first, ExchangeRate-API provider fallback, approved `admin_manual` quote/read fallback | Provider capturedAt age <= 300 seconds for quote/read workflows | Provider capturedAt age <= 60 seconds; no admin_manual fallback | Provider capturedAt age <= 300 seconds, else admin fallback if available | Provider capturedAt age <= 300 seconds, else admin fallback if available | Inherited from snapshot source | latest valid stored row with `effectiveAt <= Season.endAt`; no execute freshness window | GO for provider quote/read, execute, daily snapshot, and settlement allowlist | Official/reference source can still be evaluated later |
| Domestic stock KRX   | `admin_manual` fallback plus KIS KRX provider_api for approved workflows | Provider capturedAt age <= 60 seconds for approved quote/read workflows | Provider capturedAt age <= 10 seconds; no admin_manual fallback | Provider capturedAt age <= 60 seconds, else admin fallback if available | Provider capturedAt age <= 60 seconds, else admin fallback if available | Inherited from daily snapshots | latest valid stored row with `effectiveAt <= Season.endAt`; no execute freshness window | GO for read-only/quote, orders execute, daily snapshot, and settlement allowlist | Official close source for final evidence                  |
| US stock             | `admin_manual` fallback plus KIS US provider_api for approved workflows  | Provider capturedAt age <= 60 seconds for approved quote/read workflows | Provider capturedAt age <= 10 seconds; no admin_manual fallback | Provider capturedAt age <= 60 seconds, else admin fallback if available | Provider capturedAt age <= 60 seconds, else admin fallback if available | Inherited from daily snapshots | latest valid stored row with `effectiveAt <= Season.endAt`; no execute freshness window | GO for read-only/quote, orders execute, daily snapshot, and settlement allowlist | Delayed data acceptance for broader workflows             |
| Crypto               | `admin_manual` fallback plus Binance provider_api for approved workflows | Provider capturedAt age <= 60 seconds for approved quote/read workflows | Provider capturedAt age <= 10 seconds; no admin_manual fallback | Provider capturedAt age <= 60 seconds, else admin fallback if available | Provider capturedAt age <= 60 seconds, else admin fallback if available | Inherited from daily snapshots | latest valid stored row with `effectiveAt <= Season.endAt`; no execute freshness window | GO for read-only/quote, orders execute, daily snapshot, and settlement allowlist | USDT-to-USD policy remains MVP-only; depeg not modeled    |
