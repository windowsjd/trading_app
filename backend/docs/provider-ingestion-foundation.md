...
- A lifecycle-owned KRX startup catch-up reuses that REST current-price service once when the process starts after today's real KRX close and active domestic assets lack latest-completed-session coverage. It skips pre-open, live sessions, weekends/holidays, missing calendar coverage, disabled providers, and already-covered sessions; failures are non-fatal. This does not enable polling and does not replace the long-lived WebSocket.
