...
3. `docs/trading-modes-and-accounts.md` — season/general investment mode rules, the general-mode one-time 10M KRW grant + rewarded-ad funding policy, the TradingAccount foundation contract (partial unique index, backfill mapping, join transaction), the deploy-boundary null-link repair layer (deterministic account id, `pnpm trading-accounts:repair-links` dry-run/apply script), the exclusion→account-suspended sync, the trading-accounts read API + ownership layer, and what is NOT implemented yet (general-mode APIs, accountId migration of trading tables, ad rewards, time-weighted returns).
