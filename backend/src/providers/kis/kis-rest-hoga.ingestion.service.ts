...
import {
  KIS_DOMESTIC_HOGA_SOURCE_NAME,
  KIS_US_DELAYED_HOGA_SOURCE_NAME,
  type KisRestHogaIngestionResult,
  type KisRestHogaSnapshot,
  type KisRestHogaSummary,
} from './kis-rest-hoga.types';
...
export type KisRestHogaIngestionOptions = {
  dryRun?: boolean;
  requestedBy?: string;
...
type KisRestTargetBuildResult = {
  targets: KisRestHogaTarget[];
  skipped: KisRestHogaSummary[];
};
...
@Injectable()
export class KisRestHogaIngestionService {
  constructor(
  ...
  async ingestHogaSnapshots(
    options: KisRestHogaIngestionOptions = {},
  ): Promise<KisRestHogaIngestionResult> {
    ...
    try {
      ...
      const summaries: KisRestHogaSummary[] = [...targets.skipped];
  ...
  private async ingestOneTarget(input: {
    target: KisRestHogaTarget;
    config: ProviderConfig;
    dryRun: boolean;
    requestedBy?: string;
    accessToken: string;
    secrets?: readonly string[];
  }): Promise<KisRestHogaSummary> {
    try {
      const fetched =
        input.target.kind === 'domestic'
          ? await this.quoteClient.getMarketDataByExplicitPath<unknown>({
              path: input.config.kis.restDomesticHogaPath,
              query: {
              ...
              headers: {
                ...
                tr_id: input.config.kis.restDomesticHogaTrId,
                custtype: input.config.kis.wsCustType,
              },
            })
          : await this.quoteClient.getMarketDataByExplicitPath<unknown>({
              path: input.config.kis.restUsHogaPath,
              query: {
                AUTH: '',
              ...
              headers: {
                authorization: `Bearer ${input.accessToken}`,
                tr_id: input.config.kis.restUsHogaTrId,
                custtype: input.config.kis.wsCustType,
              },
            });
      ...
      const sourceName = sourceNameForSnapshot(parsed);
      ...
      await this.prisma.assetOrderbookSnapshot.create({
        data: {
          ...
          sourceName,
          bidPrice: parsed.bidPrice,
          bidQuantity: parsed.bidQuantity,
          askPrice: parsed.askPrice,
          askQuantity: parsed.askQuantity,
          spreadBps: parsed.spreadBps,
  ...
  private async buildTargets(
    ...
    const targets: KisRestHogaTarget[] = [];
    const skipped: KisRestHogaSummary[] = [];
  ...
  private async findMappedAsset(
    snapshot: KisRestHogaSnapshot,
  ): Promise<KisAssetMapping> {
    if (snapshot.kind === 'domestic_krx_hoga') {
      return this.findMappedDomesticAsset(snapshot.symbol);
    }
  ...
  private async findMappedUsAsset(input: {
    ...
    if (!marketCode) {
      return skipped('US_MARKET_NOT_ALLOWED');
    }
  ...
  private async findDuplicateSnapshot(input: {
    assetId: string;
    parsed: KisRestHogaSnapshot;
    sourceName: string;
  }) {
    return this.prisma.assetOrderbookSnapshot.findFirst({
      where: {
        assetId: input.assetId,
        sourceType: AssetPriceSourceType.provider_api,
        sourceName: input.sourceName,
        bidPrice: input.parsed.bidPrice,
        askPrice: input.parsed.askPrice,
...
function assertKisRestIngestionEnabled(config: ProviderConfig): void {
  ...
  if (!config.kis.restBaseUrl) {
    throw new ProviderConfigError(
      ...
      'KIS_REST_BASE_URL is required for KIS REST hoga ingestion.',
    );
  }
}
...
function skippedTarget(
  ...
  return emptySkipped(target.symbol, sourceNameForTarget(target), reason);
}

function sourceNameForSnapshot(
  snapshot: KisRestHogaSnapshot,
):
  | typeof KIS_DOMESTIC_HOGA_SOURCE_NAME
  | typeof KIS_US_DELAYED_HOGA_SOURCE_NAME {
  return snapshot.kind === 'domestic_krx_hoga'
    ? KIS_DOMESTIC_HOGA_SOURCE_NAME
    : KIS_US_DELAYED_HOGA_SOURCE_NAME;
}
...
function countAcceptedSnapshots(
  summaries: readonly KisRestHogaSummary[],
): number {
...
function resultFromSummaries(input: {
  dryRun: boolean;
  received: number;
  summaries: KisRestHogaSummary[];
}): KisRestHogaIngestionResult {
  return {
