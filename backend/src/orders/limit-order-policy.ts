...
/**
 * Pure calculation/constants for the limit-order reservation foundation.
...
export type LimitSellQuoteAmounts = {
  grossAmount: Prisma.Decimal;
  feeAmount: Prisma.Decimal;
  netAmount: Prisma.Decimal;
};

/** Limit-sell proceeds at the pinned quote fee (gross - fee). */
export function calculateLimitSellQuote(input: {
  limitPrice: Prisma.Decimal;
  quantity: Prisma.Decimal;
  tradeFeeRate: Prisma.Decimal;
}): LimitSellQuoteAmounts {
  const grossAmount = roundDecimalHalfUp(
    input.quantity.mul(input.limitPrice),
    monetaryScale,
  );
